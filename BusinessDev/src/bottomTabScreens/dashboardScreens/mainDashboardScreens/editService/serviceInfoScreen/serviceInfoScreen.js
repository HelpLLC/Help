...
export default function profileScreen(props) {
	...
	function formatDate(date) {
		return `${weekdays[date.getDay()].substring(0, 3).toUpperCase()}, ${
			...
			if (businessInfo.businessHours[weekdays[i]] != undefined) {
				days.push(
					<Text style={style.ServiceText}>
						{weekdays[i].substring(0, 1).toUpperCase() + weekdays[i].substring(1) + 's \t'}
					</Text>
				);
				let times = formatTime(
					businessInfo.businessHours[weekdays[i]].from,
					businessInfo.businessHours[weekdays[i]].to
				).split(' - ');
