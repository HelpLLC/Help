...
import style from './calendarScreenStyle';
...
export default function calendarScreen(props) {
    ...
    async function getData(){
        ...
        let start = today.toJSON();
        // today.setMonth(today.getMonth()+2);
        today.setFullYear(2100);
        let limit = today.toJSON();
        const scheduleArr = await FirebaseFunctions.call('getBusinessCurrentRequestsByTimeFrame', {
            businessID:BID,
            start,
            limit
        });
        ...
        for(let i in scheduleArr){
            if(scheduleObj[scheduleArr[i].date] != undefined)
                scheduleObj[scheduleArr[i].date].push(scheduleArr[i]);
            else scheduleObj[scheduleArr[i].date] = [scheduleArr[i]];
        }
