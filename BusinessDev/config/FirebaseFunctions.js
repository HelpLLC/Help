...
export default class FirebaseFunctions {
	...
	//user can log in
	static async addProviderToDatabase(newProvider) {
		const batch = this.database.batch();
		const uid = newProvider.providerID;
		...
		this.functions.httpsCallable('sendNewBusinessEmail')({
			...
			providerID: uid,
			businessEmail: newProvider.email,
	...
	static async deleteRequest(productID, requesterID) {
		...
		this.functions.httpsCallable('sendNotification')({
			...
			body:
				strings.YourRequestFor + ' ' + doc.data().serviceTitle + ' ' + strings.HasBeenRemovedBecause
		});
