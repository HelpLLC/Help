    ...
    render() {
        return (
            ...
            <HelpView style={screenStyle.container}>
                        ...
                        <RoundBlueButton
                            ...
                            onPress={() => {
                              if(this.state.email.trim().length === 0) {
                                 ...
                                 FirebaseFunctions.forgotPassword(this.state.email).then(() => {
                                    this.setState({ emailedLink: true, accountDNE: false})
                                 })
                                 .catch((error) => {
                                    this.setState({ emailedLink: false, accountDNE: true})
                                 });
                ...
                />
                <ErrorAlert
                    isVisible={this.state.accountDNE}
                    onPress={() => { this.setState({ accountDNE: false })}}
                    title={strings.Whoops}
                    message={strings.AccountDNE}
                />
            </HelpView>
        );
    }
