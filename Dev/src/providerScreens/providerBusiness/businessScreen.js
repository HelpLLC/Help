...
class businessScreen extends Component {
  ...
  async fetchDatabaseData() {
    try {
      ...
      if (provider.serviceIDs.length === 0) {
        ...
        provider.serviceIDs.length !== this.state.providerProducts.length
      ) {
        ...
        for (const ID of serviceIDs) {
          ...
          newArrayOfProducts.sort((productA, productB) => {
            return (
              productB.requests.currentRequests.length -
              productA.requests.currentRequests.length
            );
          });
          this.setState({
  ...
  async componentDidMount() {
    ...
    this.willFocusListener = await this.props.navigation.addListener(
      "willFocus",
      async () => {
    ...
    if (isLoading === true) {
      ...
      return (
        <HelpView style={screenStyle.container}>
          ...
          />
          <View
            style={{ height: Dimensions.get("window").height * 0.025 }}
          ></View>
          <OptionPicker
