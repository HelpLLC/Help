...
export default class orderHistoryScreen extends Component {
	...
	render() {
		...
		if (isLoading === true) {
			...
			return (
				<SideMenu isOpen={this.state.isOpen} menu={<LeftMenu navigation={this.props.navigation} allProducts={this.props.navigation.state.params.allProducts} requester={requester} />}>
								...
								serviceObjectsInProgress.length > 0 ? (
									<View>
										<View
											...
											<Text style={fontStyles.bigTextStyleBlue}>{strings.InProgress + ' (' + serviceObjectsInProgress.length + ')'}</Text>
										</View>
										<NarrowServiceCardList requester={requester} navigation={this.props.navigation} services={serviceObjectsInProgress} dateRequested={true} />
									</View>
								) : (
									<View></View>
								)}
